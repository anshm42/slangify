...
def test_footer_labels_source():
    ...
    assert "Claude → Urban Dictionary" in embed.footer.text
...
def test_caps_fields_and_adds_more_marker():
    embed = build_embed(_defs(MAX_FIELDS + 3), "claude")
...
def test_no_more_marker_when_within_cap():
    embed = build_embed(_defs(MAX_FIELDS), "claude")
...
def test_truncates_long_value():
    ...
    embed = build_embed([Definition(term="t", definition=long_def)], "claude")
